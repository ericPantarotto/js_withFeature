...
class RecipeView extends View {
  ...
  // NOTE: https://www.npmjs.com/package/js-datepicker
  addHandlerCalendar(handler) {
    const picker = datepicker('.calendar-list__add-btn', {
      position: 'br',
      onSelect: instance => {
        // console.log(instance.dateSelected);
        handler(instance.dateSelected);
      },
      disabler: date => {
        const dt = new Date();
        return date < dt.setDate(dt.getDate() - 1);
      },
      // defaultView: 'overlay',
      // overlayButton: 'Go!',
  ...
  _generateHTML() {
    return `<figure class="recipe__fig">
          ...
          } alt="Picture of a descibed recipe" class="recipe__img" />
      ...
      this._data.bookmarked ? '-fill' : ''
        ...
        <div class="recipe__ingredients">
          ...
          ${this._data.ingredients
            .map(ingredient => this._generateIngredientHTML(ingredient))
            .join(' ')}
    ...
    return `<li class="recipe__ingredient">
        ...
        <div class="recipe__text">
          ...
          <div class="recipe__description">
            ...
            <span class="recipe__description--description">${
  ...
  addHandlerAllIngredients(handlerFunction) {
    this._parentElement.addEventListener(
      ...
      function (e) {
        ...
        const ingredientList = this._parentElement.querySelectorAll(
          '.recipe__ingredient'
        );
        const ingredientsData = [];
        ingredientList.forEach(ingredient => {
          const extractedIngredient = this._getIngredient(
            addAllIngredientBtn,
            ingredient
          );
          ingredientsData.push(extractedIngredient);
        });
        ingredientsData.forEach(dataSet =>
          handlerFunction(dataSet[0], dataSet[1], dataSet[2])
        );
      }.bind(this)
    );
  }
  ...
  _getIngredient(button, ingredient = button.closest('.recipe__ingredient')) {
    ...
    const description = ingredient.querySelector(
      '.recipe__description--description'
    ).textContent;
